...
& $Python (Join-Path $Bundle "portable_launcher.py") write-process-record --package-root $Root --record-path (Join-Path $Root "data\local\run\worker.pid.json") --pid $process.Id --parent-pid $PID --process-created-at $created --executable $Python --port $Port --build-id $buildId -- @arguments
